...
if (model==1 & ps1==0 & est==2) {
Theta[i]=(thmle[,2]*ip[j,3])+log((1/Nopt)/(1-(1/Nopt)))
}
if (model==2 & ps1==0 & est==2) {
Theta[i]=(thbme[,2]*ip[j,3])+log((1/Nopt)/(1-(1/Nopt)))
}
break
}
}
...
write.csv(Theta, "C:\\RBIRT\\th.csv", row.names=TRUE)
return(Results)
}